...
public class Electron {
    ...
    static double kappa = Math.sqrt(2*mass_e*energyDiff/Math.pow(rPlanck_const,2)); //Power law index scaling factor
    ...
    public Boolean notGateTunnel() { 
        if (tunProb >= random.nextDouble()) { //generate random number 0-1, compare to tunneling probability
            tunNum += 1;
            gate += "NOT ";
            return true;
        } else {
    ...
    public Boolean andGateTunnel(Boolean bit1, Boolean bit0) { //Arguments are false if the bit being operated upon is 0 
        double randProb;
        if (bit1 == false && bit0 == false) { // If both bits are set to 0 (False), the electron must tunnel through two transistor barriers to cause an error
            randProb = random.nextDouble() * random.nextDouble(); // Random probability for first barrier * random probability for 2nd barrier
        } else { // If at least 1 bit is not 0, the electron only needs to tunnel through one barrier
